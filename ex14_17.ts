...
for (let i = 0; i < guestList.length; i++) {
  console.log(
    `Mr.${
      guestList[i] + ", "
    }I would like to formally invite you to Dinner. If you can make it.`
  );
  //for Ex14
}
console.log("\n");
...
for (let j = 7; j > 0; j--) {
  ...
  console.log(
    `Mr.${notInvited}, I'd like to apologise, there was a mishap and the Dinner is postpond until further notice.`
  );
}
console.log("\n");
for (let i = 0; i < guestList.length; i++) {
  console.log(
    `Mr.${
      guestList[i] +
      ", I would like to formally Re-invite you to Dinner. If you can still make it."
    }`
  );
}
